...
public class SchemProduction {
    ...
    public static boolean isMineableItem(Item item) {
    ...
    public static boolean isUnitBlock(Block block) {
        return block instanceof mindustry.world.blocks.units.UnitBlock 
    ...
    public static Result compute(Schematic schem) {
        ...
        for (var tile : schem.tiles) {
            ...
            if (block == null || isUnitBlock(block)) continue;
            ...
            if (isModifierBlock(block)) {
                processModifierBlock(block, craftTime, boostGroups);
                customItemConsumersProcessed = true;
            }

            // --- PRODUCTION OUTPUTS ---
            processProductionOutputs(r, block, tile, drillGroups);

            // --- DRILL LIQUID BOOSTS ---
            processDrillBoosts(block, boostGroups);

            // --- GENERATORS WITH FILTERED FUELS ---
            if (processGeneratorFuels(block, genGroups)) {
                customItemConsumersProcessed = true;
            }

            // --- CONSUMPTION VIA BLOCK CONSUMERS ---
            processStandardConsumers(r, block, craftTime, customItemConsumersProcessed, boostGroups);
        }

        r.generatorFuels.addAll(genGroups.values().toSeq());
        r.drillOptions.addAll(drillGroups.values().toSeq());
        r.optionalBoosts.addAll(boostGroups.values().toSeq());

        return r;
    }

    private static void processModifierBlock(Block block, float craftTime, ObjectMap<Block, OptionalBoostGroup> boostGroups) {
    ...
    private static void processProductionOutputs(Result r, Block block, Schematic.Stile tile, ObjectMap<Block, DrillOptionGroup> drillGroups) {
        if (block instanceof Separator separator) {
            ...
            processDrillOutput(r, drill, tile, drillGroups);
        } else if (block instanceof BeamDrill beamDrill) {
            processBeamDrillOutput(r, beamDrill, tile, drillGroups);
        }
    }

    private static void processDrillOutput(Result r, Drill drill, Schematic.Stile tile, ObjectMap<Block, DrillOptionGroup> drillGroups) {
        if (tile.config instanceof Item item) {
            ...
            DrillOptionGroup group = drillGroups.get(drill);
            if (group == null) {
                group = new DrillOptionGroup();
                group.drill = drill;
                group.count = 1;
                drillGroups.put(drill, group);
    ...
    private static void processBeamDrillOutput(Result r, BeamDrill beamDrill, Schematic.Stile tile, ObjectMap<Block, DrillOptionGroup> drillGroups) {
        if (tile.config instanceof Item item) {
            ...
            DrillOptionGroup group = drillGroups.get(beamDrill);
            if (group == null) {
                group = new DrillOptionGroup();
                group.drill = beamDrill;
                group.count = 1;
                drillGroups.put(beamDrill, group);
    ...
    private static void processDrillBoosts(Block block, ObjectMap<Block, OptionalBoostGroup> boostGroups) {
    ...
    private static boolean processGeneratorFuels(Block block, ObjectMap<Block, GeneratorFuelGroup> genGroups) {
    ...
    private static void processStandardConsumers(Result r, Block block, float craftTime, boolean customItemConsumersProcessed, ObjectMap<Block, OptionalBoostGroup> boostGroups) {
        if (block.consumers == null) return;
        ...
        for (Consume c : block.consumers) {
            ...
            if (c instanceof ConsumeItems ci) {
                if (!customItemConsumersProcessed) {
                    for (ItemStack in : ci.items) {
                        ...
                        if (isBoostOrOptional) {
                            addOptionalItemBoost(boostGroups, block, in.item, rate);
                        } else {
                ...
                if (!isExemptLiquidConsumer(block)) {
                    if (cl.liquid != null) {
                        float rate = cl.amount * 60f;
                        if (isBoostOrOptional) {
                            addOptionalLiquidBoost(boostGroups, block, cl.liquid, rate);
                        } else {
                ...
                if (!isExemptLiquidConsumer(block)) {
                    for (LiquidStack in : cls.liquids) {
                        ...
                        if (isBoostOrOptional) {
                            addOptionalLiquidBoost(boostGroups, block, in.liquid, rate);
                        } else {
                ...
                if (!isExemptLiquidConsumer(block)) {
                    ...
                    if (Vars.content != null && Vars.content.liquids() != null) {
                        for (Liquid liq : Vars.content.liquids()) {
                            if (clf.filter.get(liq)) {
                                if (isBoostOrOptional) {
                                    addOptionalLiquidBoost(boostGroups, block, liq, rate);
                                } else {
    ...
    private static boolean isExemptLiquidConsumer(Block block) {
        return block instanceof Drill || block instanceof BeamDrill || isModifierBlock(block);
    }

    private static void addOptionalItemBoost(ObjectMap<Block, OptionalBoostGroup> boostGroups, Block block, Item item, float rate) {
        OptionalBoostGroup group = boostGroups.get(block);
        if (group == null) {
            group = new OptionalBoostGroup();
            group.block = block;
        ...
        group.itemRates.increment(item, 0f, rate);
    }

    private static void addOptionalLiquidBoost(ObjectMap<Block, OptionalBoostGroup> boostGroups, Block block, Liquid liquid, float rate) {
        OptionalBoostGroup group = boostGroups.get(block);
        if (group == null) {
            group = new OptionalBoostGroup();
            group.block = block;
        ...
        group.liquidRates.increment(liquid, 0f, rate);
    }
